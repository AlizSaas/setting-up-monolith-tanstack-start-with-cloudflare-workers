import { z } from 'zod';
import { INVOICE_STATUSES, CURRENCIES, DISCOUNT_TYPES, PAYMENT_TERMS, INVOICE_EVENT_TYPES, TIMEZONES, REMINDER_TYPES } from './constant';


// --- Helpers for Postgres Compatibility ---

// Postgres 'numeric' returns strings. We coerce them to numbers for the app logic.
const moneySchema = z.coerce.number();

// Postgres 'timestamp' returns Date objects. We ensure they become ISO strings for the app.
...
export const invoiceItemSchema = z.object({
  ...
  quantity: moneySchema.positive('Quantity must be positive'),
  unit_price: moneySchema.min(0, 'Unit price must be non-negative'),
  tax_rate: moneySchema.min(0).max(100).nullable().optional(),
  amount: moneySchema,
...
export const createInvoiceItemSchema = z.object({
  ...
  quantity: moneySchema.positive('Quantity must be positive'),
  unit_price: moneySchema.min(0, 'Unit price must be non-negative'),
  tax_rate: moneySchema.min(0).max(100).optional().nullable(),
...
export const invoiceSchema = z.object({
  ...
  // Coerced from Postgres numeric string -> number
  subtotal: moneySchema,
  tax_total: moneySchema,
  discount_type: discountTypeSchema.nullable().optional(),
  discount_value: moneySchema.min(0).nullable().optional(),
  discount_amount: moneySchema.min(0).nullable().optional(),
  total: moneySchema,
  
  notes: z.string().max(2000).nullable().optional(),
  payment_terms: paymentTermsSchema.nullable().optional(),
  reminders_enabled: z.boolean().default(true),
  pdf_generated_at: dateSchema.nullable().optional(),
  created_at: dateSchema,
  updated_at: dateSchema,
});
...
export const createInvoiceSchema = z.object({
  ...
  issue_date: z.string(), // Input is still string from form
  due_date: z.string(),   // Input is still string from form
  currency: currencySchema,
...
export const dashboardKPIsSchema = z.object({
  total_outstanding: moneySchema,
  total_paid_this_month: moneySchema,
...
export const reminderSchema = z.object({
  ...
  sent_at: dateSchema.nullable().optional(),
});
