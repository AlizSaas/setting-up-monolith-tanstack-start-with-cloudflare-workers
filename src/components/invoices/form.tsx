...
import { createInvoiceSchema } from '@/lib/schema.type';
...
export function InvoiceFormPage({invoiceId}: {invoiceId?: string}) {
  ...
  const { data: clients, isLoading: isLoadingClients } = useQuery(clientsQueryOptions());
  ...
  const {
    ...
    defaultValues: {
      currency: 'USD',
      items: [{ description: '', quantity: 1, unit_price: 0 }], // Start with one empty item
    },
  });
  ...
  useEffect(() => {
    if (settings && !isEditing) {
      setValue('currency', settings.defaultCurrency as any);
      setValue('payment_terms', settings.defaultPaymentTerms as any);
      ...
      const days =
        {
          ...
          net_60: 60,
          net_90: 90,
        }[settings.defaultPaymentTerms as keyof typeof paymentTermsDays] || 30;
...
const createMutation = useMutation({
  ...
  onSuccess: (data) => {
    queryClient.invalidateQueries({ queryKey: INVOICES_QUERY_KEY.invoicesInvalidation });
    toast.success("Invoice created successfully");
    // Ensure you use the returned data.id
